...
from bert4keras.models import NEZHA, RoFormer
...
class MultiHeadAttention(Layer):
    ...
    def pay_attention_to(self, inputs):
        ...
        if "past_key" in inputs and "past_value" in inputs:
            ...
            past_key_mask = inputs['past_key_mask']
            past_value_mask = inputs['past_value_mask']

            add_a = tf.einsum('bjhd,bkhd->bhjk', qw, past_key)

            if self.attention_scale:
                add_a = add_a / self.key_size**0.5
            
            add_a = sequence_masking(add_a, past_value_mask, '-inf', -1)
            # 合并
            # bhj[add_k+k] 
            A = K.softmax(K.concatenate((add_a, a), axis=-1))
            if self.attention_dropout:
                A = Dropout(self.attention_dropout)(A)

            # 合并
            # b[add_k+k]hd
            o = tf.einsum('bhjk,bkhd->bjhd', A, K.concatenate([past_value, vw], axis=1))

            # A 拆分
            add_a_k_len = K.shape(add_a)[-1]
            a_k_len = K.shape(a)[-1]

            ADD_A, A = tf.split(A, [add_a_k_len, a_k_len], axis=-1)
...
class PtuningV2AddFNNPtuning(RoFormer):
    ...
    def apply_main_layers(self, inputs, index):
        ...
        # Self Attention
        xi, x, x_names = x, [x, x, x, position_bias], ["q", "k", "v", "rotary_p_bias"]
